...
pub fn unwrap_ddg_redirect(href: &str) -> Option<String> {
    ...
    // Relative protocol → https for parsing.
    ...
    // Prefer uddg query param when present (redirect wrapper).
...
fn sanitize_result_url(raw: &str) -> Option<String> {
    ...
    // Drop DDG itself, ad click trackers, and obvious junk.
...
pub fn search_result_urls(
    ...
    for page in 0..pages {
        ...
        let html = fetcher
            .fetch(&search_url)
            .with_context(|| format!("fetching DuckDuckGo results for '{query}' (page {page})"))?;
        let found = parse_duckduckgo_results(&html)
            .with_context(|| format!("parsing DuckDuckGo results for '{query}' (page {page})"))?;
        if page == 0 && found.is_empty() && !is_duckduckgo_challenge(&html) {
...
pub fn search_scrape_recipes(
    ...
    progress(ScrapeEvent::Planned {
        ...
        skipped: seeds
            .iter()
            .filter(|u| skip.contains(&normalize_url(u)))
            .count(),
        to_fetch: seeds
            .iter()
            .filter(|u| !skip.contains(&normalize_url(u)))
...
mod tests {
    ...
    impl HtmlFetcher for MapFetcher {
        fn fetch(&self, url: &str) -> Result<String> {
            // Match ignoring minor query order differences for search URLs.
            if let Some(html) = self.pages.get(url) {
                return Ok(html.clone());
            }
            // Allow prefix match on DDG endpoint.
            for (k, v) in &self.pages {
                if (url.starts_with(k) || k.starts_with(url.split('?').next().unwrap_or(url)))
                    && (k.contains("duckduckgo") || url.contains("duckduckgo"))
                {
                    return Ok(v.clone());
                }
            }
    ...
    fn search_scrape_multi_host_bfs() {
        ...
        pages.insert(
            "https://html.duckduckgo.com/html/?q=breakfast+recipes".to_string(),
            ddg.clone(),
        );
        // Also accept URL as built by duckduckgo_search_url
